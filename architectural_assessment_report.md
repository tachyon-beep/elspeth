...
- **Metrics plugins**: Row/aggregation/baseline plugins covering extraction, stats, power analysis, agreement, and Bayesian comparisons with optional SciPy/Pingouin integrations (`src/elspeth/plugins/experiments/metrics.py:144`, `src/elspeth/plugins/experiments/metrics.py:258`, `src/elspeth/plugins/experiments/metrics.py:403`).
- **Outputs**: CSV, blob, signed bundle, zip, Excel, GitHub, Azure DevOps sinks with dry-run safety and artifact manifest support (`src/elspeth/plugins/outputs/csv_file.py:16`, `src/elspeth/plugins/outputs/repository.py:41`).
...
### Key Components
- **Monolithic CLI**: Direct Azure OpenAI client usage, global state, and imperative orchestration with tight coupling to other `elspeth` subpackages (`old/main.py:32`).
...
- **Concurrency & retry**: Thread pool orchestration for case-study processing with manual backoff wrappers referencing `elspeth.llm_client` helpers (`old/main.py:206`).
...
- Heavy reliance on local `elspeth.*` modules (rate_limit, prompts, heuristics, monitoring) without an explicit packaging boundary.
...
|------------------|-------------|-------------|---------|-------|
| CLI orchestration | Monolithic script with global state (`old/main.py:32`) | Modular CLI invoking orchestrator & suite runner (`src/elspeth/cli.py:91`) | Refactored | Clear separation of validation, single-run, and suite paths. |
| Configuration loading | JSON/YAML parsing with inline validation (`old/experiment_runner.py:41`) | Typed settings with registry-backed plugin instantiation (`src/elspeth/config.py:17`) | Modified | Adds prompt packs, suite defaults, middleware, concurrency options. |
| Prompt management | File-based prompts and manual formatting (`old/main.py:144`) | Jinja prompt engine with validation & defaults (`src/elspeth/core/prompts/engine.py:50`) | Improved | Supports template metadata, aliasing, prompt packs. |
| LLM integration | Direct AzureOpenAI calls wrapped by legacy client (`old/main.py:200`) | Protocol-based clients + middleware + cost/rate controls (`src/elspeth/plugins/llms/azure_openai.py:17`, `src/elspeth/core/experiments/runner.py:316`) | Improved | Middleware pipeline enables audit, shield, Azure telemetry. |
| Retry/backoff & checkpointing | Manual wrappers via `elspeth.llm_client` and `ExperimentCheckpoint` (`old/main.py:135`) | Configurable retry/backoff and checkpoint JSONL in runner (`src/elspeth/core/experiments/runner.py:316`, `src/elspeth/core/experiments/runner.py:51`) | Added | Checkpoint resume and middleware-aware retries. |
| Concurrency control | ThreadPoolExecutor usage embedded in CLI (`old/main.py:206`) | Configurable parallelism gated by rate limiter utilisation (`src/elspeth/core/experiments/runner.py:239`) | Modified | Adds backlog thresholds, utilisation pauses, middleware safety. |
| Baseline comparison | Diff logic in suite manager with Azure ML logging (`old/experiment_runner.py:311`) | Baseline plugins plus suite runner comparisons (`src/elspeth/core/experiments/suite_runner.py:236`) | Refactored | Plugins enable multiple comparison strategies. |
