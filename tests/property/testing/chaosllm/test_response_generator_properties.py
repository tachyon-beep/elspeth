# tests/property/testing/chaosllm/test_response_generator_properties.py
...
class TestOpenAIResponseStructure:
    ...
    def test_to_dict_has_required_fields(self, content: str, model: str, prompt_tokens: int, completion_tokens: int) -> None:
...
class TestRandomMode:
    ...
    @settings(max_examples=100)
    def test_random_word_count_within_bounds(self, min_words: int, max_words: int, seed: int, vocabulary: str) -> None:
        ...
        assert min_words <= word_count <= max_words, f"Word count {word_count} outside [{min_words}, {max_words}]"
...
class TestDeterminism:
    ...
    def test_same_seed_same_response(self, seed: int) -> None:
        ...
        gen1 = ResponseGenerator(config, rng=rng1, time_func=lambda: 1700000000.0, uuid_func=lambda: "abc")

        rng2 = random.Random(seed)
        gen2 = ResponseGenerator(config, rng=rng2, time_func=lambda: 1700000000.0, uuid_func=lambda: "abc")
...
class TestGenerateIntegration:
    ...
    def test_generate_returns_valid_response(self, seed: int) -> None:
        ...
        config = ResponseConfig(mode="random")
        rng = random.Random(seed)
        gen = ResponseGenerator(config, rng=rng, time_func=lambda: 1700000000.0, uuid_func=lambda: "x")
    ...
    def test_mode_override_respected(self, mode: str, seed: int) -> None:
        ...
        config = ResponseConfig(mode="random")
        rng = random.Random(seed)
        gen = ResponseGenerator(config, rng=rng, time_func=lambda: 0.0, uuid_func=lambda: "x")
    ...
    def test_sequential_cycling_wraps(self, n_responses: int, seed: int) -> None:
        ...
        for cycle in range(3):
            for i in range(n_responses):
                result = bank.next()
                assert result == f"response_{i}", f"Cycle {cycle}, index {i}: expected response_{i}, got {result}"
