...
def test_validate_rate_limiter_unknown():
    with pytest.raises(ConfigurationError):
        controls_registry.validate_rate_limiter({"plugin": "missing", "security_level": "official"})
...
def test_create_cost_tracker_unknown_plugin():
    with pytest.raises(ValueError):
        controls_registry.create_cost_tracker({"plugin": "unknown", "security_level": "official"})


def test_validate_cost_tracker_success():
    controls_registry.validate_cost_tracker({"plugin": "noop", "security_level": "official"})
...
def test_create_rate_limiter_unknown_plugin():
    with pytest.raises(ValueError):
        controls_registry.create_rate_limiter({"plugin": "unknown", "security_level": "official"})


def test_validate_rate_limiter_success():
    controls_registry.validate_rate_limiter({"plugin": "noop", "security_level": "official"})
