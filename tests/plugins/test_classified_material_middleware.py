...
def test_abort_on_secret_literal():
    mw = ClassifiedMaterialMiddleware(on_violation="abort")
...
def test_mask_mode_masks_literal_case_insensitive():
    mw = ClassifiedMaterialMiddleware(on_violation="mask", mask="[MASK]")
...
def test_severity_threshold_allows_medium_when_min_high():
    mw = ClassifiedMaterialMiddleware(on_violation="abort", severity_scoring=True, min_severity="HIGH")
...
def test_allcaps_confidence_blocks_lowercase_without_context():
    mw = ClassifiedMaterialMiddleware(on_violation="abort", require_allcaps_confidence=True, case_sensitive=True)
...
def test_false_positive_suppressed_inside_code_fence():
    mw = ClassifiedMaterialMiddleware(on_violation="abort", check_code_fences=True)
    text = """
    ```
...
def test_log_mode_with_regex_detection_returns_request():
    mw = ClassifiedMaterialMiddleware(on_violation="log")
