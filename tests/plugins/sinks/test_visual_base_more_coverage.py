...
def test_visual_base_validation_and_save(tmp_path: Path):
    sink = BaseVisualSink(
        ...
        dpi=100,
        figure_size=(2, 1),
        security_level="UNOFFICIAL",  # ADR-002-B: Required for base class tests
...
def test_visual_base_update_security_context():
    sink = BaseVisualSink(base_path=".", file_stem="x", security_level="UNOFFICIAL", allow_downgrade=True)
...
def test_visual_base_invalid_params():
    ...
    with pytest.raises(ValueError):
        BaseVisualSink(base_path=".", file_stem="x", dpi=0, security_level="UNOFFICIAL", allow_downgrade=True)
    # Invalid figure size
    with pytest.raises(ValueError):
        BaseVisualSink(base_path=".", file_stem="x", figure_size=(0, 1), security_level="UNOFFICIAL", allow_downgrade=True)
    with pytest.raises(ValueError):
        BaseVisualSink(base_path=".", file_stem="x", figure_size=(1,), security_level="UNOFFICIAL", allow_downgrade=True)  # type: ignore[arg-type]
    # Invalid on_error
    with pytest.raises(ValueError):
        BaseVisualSink(base_path=".", file_stem="x", on_error="noop", security_level="UNOFFICIAL", allow_downgrade=True)
