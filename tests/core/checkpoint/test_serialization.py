...
class TestCheckpointSerialization:
    ...
    def test_naive_datetime_gets_utc(self) -> None:
        ...
        # Create a naive datetime by removing timezone from a tz-aware one
        aware_dt = datetime(2026, 2, 5, 12, 30, 45, tzinfo=UTC)
        naive_dt = aware_dt.replace(tzinfo=None)  # Now naive (no timezone)
        data = {"created_at": naive_dt}

        result = checkpoint_loads(checkpoint_dumps(data))

        assert result["created_at"].tzinfo is not None
        # Value should be unchanged (just with timezone added)
