...
def test_plugin_context_non_empty_fields():
    ...
    # security_level cannot be empty (string present but empty)
    with pytest.raises(ValueError):
        PluginContext(plugin_name="name", plugin_kind="kind", security_level="   ")
...
def test_plugin_context_determinism_level_validation_and_default():
    ...
    assert ctx.determinism_level == "none"

    # Valid values normalize to lowercase
    ctx2 = PluginContext(
        plugin_name="p2", plugin_kind="k2", security_level="OFFICIAL", determinism_level="HIGH"
    )
    assert ctx2.determinism_level == "high"
...
def test_plugin_context_derive_inherits_and_overrides():
    ...
    assert child.security_level == "PROTECTED"
    assert child.determinism_level == "low"
    assert child.parent == parent

    # Override when provided
    grandchild = child.derive(
        plugin_name="row_plugin", plugin_kind="row_plugin", security_level="SECRET", determinism_level="guaranteed"
    )
    assert grandchild.security_level == "SECRET"
    assert grandchild.determinism_level == "guaranteed"
