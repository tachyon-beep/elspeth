# tests/performance/stress/test_concurrent_sinks.py
"""Concurrent sink write stress tests.

Tests that multiple sink instances writing concurrently do not lose data
or corrupt ordering. Uses CollectSink from tests.fixtures.plugins
...
class TestConcurrentSinks:
    ...
    def test_concurrent_sink_writes_no_data_loss(self) -> None:
        ...
        threads = [threading.Thread(target=writer, args=(sinks[i], all_row_sets[i]), name=f"writer-{i}") for i in range(num_sinks)]
        ...
        for i, sink in enumerate(sinks):
            assert len(sink.results) == rows_per_sink, f"Sink {i} got {len(sink.results)} rows, expected {rows_per_sink}"

        # Verify total row count across all sinks
        total = sum(len(s.results) for s in sinks)
        assert total == num_sinks * rows_per_sink, f"Total rows: {total}, expected {num_sinks * rows_per_sink}"

        # Verify no cross-sink contamination (each sink only has its prefix)
        for i, sink in enumerate(sinks):
            prefix = f"sink{i}"
            for row in sink.results:
                assert row["id"].startswith(prefix), f"Sink {i} contains foreign row: {row['id']}"
    ...
    def test_concurrent_sink_ordering(self) -> None:
        ...
        threads = [threading.Thread(target=writer, args=(i,), name=f"ordered-writer-{i}") for i in range(num_threads)]
        ...
        assert len(sink.results) == expected_total, f"Got {len(sink.results)} rows, expected {expected_total}"
        ...
        for tid in range(num_threads):
            ...
            assert len(thread_rows) == rows_per_thread, f"Thread {tid} has {len(thread_rows)} rows, expected {rows_per_thread}"

            # Within each thread's rows, values should be in order
            values = [r["value"] for r in thread_rows]
            assert values == sorted(values), f"Thread {tid} rows are not in order: first 10 = {values[:10]}"
