...
class _DummyLogger:
    ...
    def log_event(self, event_type: str, *, message: str | None = None, metrics: dict | None = None, metadata: dict | None = None, level: str = "info") -> None:  # noqa: D401
        self.events.append((event_type, {"message": message, "metrics": metrics or {}, "metadata": metadata or {}, "level": level}))
...
def test_analytics_report_emits_logger_events(tmp_path: Path) -> None:
    sink = AnalyticsReportSink(base_path=str(tmp_path), file_stem="obs_test", formats=["json"], include_metadata=False, include_aggregates=False, include_comparisons=False)
