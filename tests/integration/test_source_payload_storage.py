# tests/integration/test_source_payload_storage.py
...
def _build_simple_graph(config: PipelineConfig) -> ExecutionGraph:
    ...
    graph._output_sink = next(iter(config.sinks.keys()))  # Use first sink as output
...
def test_source_row_payloads_are_stored_during_run(tmp_path: Path) -> None:
    ...
    test_source = TestSource(source_data)
    test_sink = TestSink()
