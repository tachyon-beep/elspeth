...
Migrated from tests/core/checkpoint/test_topology_validation.py.

NOTE: Manual graph construction is intentional here. These tests verify
topology hashing algorithms that need specific node IDs and configs.
"""
...
class TestCheckpointTopologyValidation:
    ...
    def test_modified_sink_config_causes_validation_failure(self) -> None:
        ...
        modified_graph.add_node(
            "sink_1",
            node_type=NodeType.SINK,
            config={"path": "DIFFERENT_OUTPUT.csv"},
            plugin_name="csv_sink",
        )
...
class TestTopologyHashSensitivity:
    """Tests that topology hash is sensitive to all structural properties."""

    def test_different_node_types_produce_different_hashes(self) -> None:
        """Changing only node_type must produce a different topology hash.

        Regression: If node_type is excluded from the hash input, swapping
        a TRANSFORM for a GATE (or vice versa) would not be detected during
...
class TestTopologyHashDeterminism:
    ...
    def test_different_configs_produce_different_hashes(self) -> None:
        ...
        graph2 = ExecutionGraph()
        graph2.add_node("source", node_type=NodeType.SOURCE, config={"x": 2}, plugin_name="csv")
    ...
    def test_different_edge_labels_produce_different_hashes(self) -> None:
        ...
        graph2.add_node("sink", node_type=NodeType.SINK, config={}, plugin_name="csv")
        graph2.add_edge("source", "sink", label="different_label", mode=RoutingMode.MOVE)
    ...
    def test_different_routing_modes_produce_different_hashes(self) -> None:
        ...
        graph2.add_node("source", node_type=NodeType.SOURCE, config={}, plugin_name="csv")
        graph2.add_node("sink", node_type=NodeType.SINK, config={}, plugin_name="csv")
        graph2.add_edge("source", "sink", label="continue", mode=RoutingMode.COPY)
...
class TestResumeAuditIntegrity:
    ...
    def test_mismatch_produces_clear_rejection_not_silent_accept(self) -> None:
        ...
        modified.add_node("transform_A", node_type=NodeType.TRANSFORM, config={"version": 2}, plugin_name="passthrough")
