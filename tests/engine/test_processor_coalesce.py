...
class TestRowProcessorCoalesce:
    ...
    def test_fork_then_coalesce_require_all(self, landscape_db: "LandscapeDB") -> None:
        ...
        class EnrichA(BaseTransform):
            ...
            def process(self, row: PipelineRow, ctx: PluginContext) -> TransformResult:
                return TransformResult.success(
                    make_pipeline_row({**row.to_dict(), "sentiment": "positive"}), success_reason={"action": "enrich"}
                )
        ...
        class EnrichB(BaseTransform):
            ...
            def process(self, row: PipelineRow, ctx: PluginContext) -> TransformResult:
                return TransformResult.success(
                    make_pipeline_row({**row.to_dict(), "entities": ["ACME"]}), success_reason={"action": "enrich"}
                )
    ...
    def test_coalesce_best_effort_with_quarantined_child(self, landscape_db: "LandscapeDB") -> None:
        ...
        sentiment_token = TokenInfo(
            ...
            row_data=make_pipeline_row({"text": "ACME earnings report", "sentiment": "positive"}),
        ...
        entities_token = TokenInfo(
            ...
            row_data=make_pipeline_row({"text": "ACME earnings report", "entities": ["ACME"]}),
    ...
    def test_coalesce_quorum_merges_at_threshold(self, landscape_db: "LandscapeDB") -> None:
        ...
        fast_token = TokenInfo(
            row_id=children[0].row_id,
            token_id=children[0].token_id,
            row_data=make_pipeline_row({"text": "test input", "fast_result": "fast done"}),
            branch_name="fast",
        )
        ...
        medium_token = TokenInfo(
            row_id=children[1].row_id,
            token_id=children[1].token_id,
            row_data=make_pipeline_row({"text": "test input", "medium_result": "medium done"}),
            branch_name="medium",
        )
        ...
        slow_token = TokenInfo(
            ...
            row_data=make_pipeline_row({"text": "test input", "slow_result": "slow done"}),
            branch_name="slow",
        )
    ...
    def test_nested_fork_coalesce(self, landscape_db: "LandscapeDB") -> None:
        ...
        inner_for_outer = TokenInfo(
            ...
            row_data=make_pipeline_row(inner_merged_token.row_data.to_dict()),
    ...
    def test_late_arrival_coalesce_returns_failed_outcome(self, landscape_db: "LandscapeDB") -> None:
        ...
        fast_token = TokenInfo(
            row_id=children[0].row_id,
            token_id=children[0].token_id,
            row_data=make_pipeline_row({"text": "test", "fast_result": "done"}),
            branch_name="fast",
        )
        slow_token = TokenInfo(
            row_id=children[1].row_id,
            token_id=children[1].token_id,
            row_data=make_pipeline_row({"text": "test", "slow_result": "done"}),
            branch_name="slow",
        )
        ...
        class BatchAggForCoalesce(BaseTransform):
            ...
            def process(self, row: PipelineRow | list[PipelineRow], ctx: Any) -> TransformResult:
                if isinstance(row, list):
                    ...
                    return TransformResult.success(PipelineRow(output_row, contract), success_reason={"action": "aggregate"})
                return TransformResult.success(make_pipeline_row(dict(row.to_dict())), success_reason={"action": "passthrough"})
