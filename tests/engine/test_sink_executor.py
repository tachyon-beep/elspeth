...
class TestSinkExecutor:
        ...
        for batch_num in range(2):
            ...
            for i in range(2):
                ...
                token = TokenInfo(
                    ...
                    row_data=make_pipeline_row({"batch": batch_num, "index": i}),
                )
                row = recorder.create_row(
                    run_id=run.run_id,
                    source_node_id=sink_node.node_id,
    ...
    def test_artifact_linked_to_first_state(self) -> None:
        ...
        for i in range(3):
            token = TokenInfo(
                row_id=f"row-{i}",
                token_id=f"token-{i}",
                row_data=make_pipeline_row({"index": i}),
            )
            row = recorder.create_row(
                run_id=run.run_id,
                source_node_id=sink_node.node_id,
    ...
    def test_flush_exception_preserves_crash_behavior(self) -> None:
        ...
        token = TokenInfo(row_id="row-0", token_id="token-0", row_data=make_pipeline_row({"x": 1}))
        row = recorder.create_row(
            run_id=run.run_id,
            source_node_id=sink_node.node_id,
            row_index=0,
