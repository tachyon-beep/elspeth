...
- **Shutdown checkpoint skip logging** — `_checkpoint_interrupted_progress` now emits a structured `shutdown_checkpoint_skipped` warning when no token/node ID can be resolved, instead of silently returning. Includes diagnostic context (aggregation state, coalesce pending, sink tokens, last token ID) for post-incident investigation.
- **Schema epoch directional guard** — `_sync_sqlite_schema_epoch` now raises `SchemaCompatibilityError` when the database has a newer epoch than the running code expects, instead of silently downgrading via `!=` comparison. Only upgrades (epoch 0 → current) are permitted.
- **Checkpoint recovery type annotation** — `_get_buffered_checkpoint_token_ids` parameter typed as `Checkpoint` instead of `Any`, enabling mypy to verify attribute accesses on checkpoint state fields.
- **Coalesce checkpoint `__post_init__` validation** — `CoalesceTokenCheckpoint` and `CoalescePendingCheckpoint` now enforce Tier 1 invariants at construction: non-empty string identifiers, non-negative timing fields, dict types for `row_data`/`contract`, and disjoint `branches`/`lost_branches` keys.
