...
class CSVBlobDataSource(BaseCSVDataSource):
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize CSV blob datasource with hard-coded security policy.

        ADR-005: allow_downgrade is set by plugin author, not configurable by users.
        CSV blob datasources default to trusted downgrade (can operate at lower security levels).
        """
        super().__init__(
            path=path,
            base_path=base_path,
            ...
            allow_downgrade=True,  # Hard-coded policy: CSV blob datasources can operate at lower levels
