...
Issues should be created at the right granularity from the start, but **retyping is encouraged** when the scope becomes clearer — a task that grows into multi-session work should be promoted to a feature or epic with child tasks, and an epic that turns out to be a single grind session should be demoted to a task.

Filigree's `update_issue` doesn't support changing types directly. To retype:

1. Create a new issue with the correct type
2. Transfer description, labels, parent, and dependencies from the old issue
3. Close the old one with a reason linking to the replacement
