# ELSPETH RC-1 Release Validation Checklist

**Purpose:** What MUST work before shipping RC-1. This is not a feature list - it's the minimum bar for release.

**Criterion:** If any item fails, RC-1 cannot ship.
...
- [ ] Coalesce timeout fires (not just at end-of-source) - **CRIT-03 in RC1 plan**
...
- [ ] TUI mode acceptable as "preview" for RC1
...
## KNOWN ISSUES ACCEPTABLE FOR RC-1
...
4. **Concurrent processing** - Config exists but not integrated (single-threaded acceptable for RC1)
5. **Profile system** - Deferred (single settings file works)
6. **Redaction profiles** - Deferred (manual redaction in plugins acceptable)

---

## BLOCKERS FROM RC1-REMEDIATION.MD

These MUST be fixed before release:

| ID | Issue | Impact |
|----|-------|--------|
| CLI-016 | `elspeth run` doesn't instantiate PayloadStore | Source payloads not persisted - violates audit requirement |
| CRIT-03 | Coalesce timeout never called in processor loop | Fork/join hangs if branch fails |
| QW-02 | Silent JSON parse fallback in HTTP client | Bad diagnostics |
