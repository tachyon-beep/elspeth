...
| Per-plugin config | every plugin __init__ | varies per plugin | **Per-plugin Config TypedDicts** (lower priority, high volume) |
...
pytest tests_v2/ -m "not performance"

# Just property tests
pytest tests_v2/property/

# Performance only
pytest tests_v2/performance/ -m performance

# Stress only (requires ChaosLLM)
pytest tests_v2/performance/stress/ -m stress
...
1. **Phase 0: Scaffolding** - Create `tests_v2/`, all conftest files, `fixtures/`, `strategies/`. Zero test files.
...
7. **Phase 6: Cutover** - Delete `tests/`, rename `tests_v2/` to `tests/`.
