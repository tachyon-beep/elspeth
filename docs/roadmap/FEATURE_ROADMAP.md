...
| **Governance & Compliance** | Signed bundles, manifest metadata | No plugin provenance attestations, approval workflow, or compliance exporter. | • Enforce plugin signing/attestation metadata.<br>• Build compliance exporter (ISM/Essential Eight evidence zip).<br>• Publish certification checklist for new plugins (see [plugin hardening principles](../development/plugin-hardening-principles.md)). |
