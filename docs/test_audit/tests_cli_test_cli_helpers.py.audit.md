# Test Audit: tests/cli/test_cli_helpers.py
...
- **Line 203-241:** `test_aggregation_rejects_transform_without_is_batch_aware_attribute` uses extensive mocking that bypasses real plugin instantiation. The mock setup at lines 217-225 creates a `MagicMock` for the transform and patches `_get_plugin_manager`, which means:
  1. The test never validates that real plugins behave correctly
  2. The mock is patching `elspeth.cli._get_plugin_manager` but `instantiate_plugins_from_config` is imported from `elspeth.cli_helpers`, so the patch location may be incorrect
  3. The mocks for source/sink (`MagicMock(return_value=MagicMock())`) are overly permissive and don't verify actual behavior
