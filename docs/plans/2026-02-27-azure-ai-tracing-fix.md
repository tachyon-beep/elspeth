...
**Architecture:** `LLMTransform.__init__()` stores the parsed `TracingConfig` as an instance field. `on_start()` calls `_configure_azure_monitor()` for `AzureAITracingConfig`. A validation check in `__init__()` rejects `azure_ai` tracing with non-Azure providers. `create_langfuse_tracer()` stops warning for Azure AI configs (tracing is active, just not via Langfuse).
...
class TestAzureAITracingSetup:
    ...
    def test_langfuse_factory_no_warning_for_azure_ai_config(self) -> None:
        ...
        with unittest.mock.patch("elspeth.plugins.llm.langfuse.logger") as mock_logger:
...
Also store the parsed config for `on_start()`:
...
```python
    def test_on_start_calls_configure_azure_monitor(self) -> None:
        ...
        with unittest.mock.patch(
            ...
            # Verify it received the AzureAITracingConfig
            call_arg = mock_configure.call_args[0][0]
    ...
    def test_on_start_skips_azure_monitor_for_langfuse(self) -> None:
        ...
        with unittest.mock.patch("elspeth.plugins.llm.transform.create_langfuse_tracer"):
            transform = LLMTransform(config)
...
Run: `.venv/bin/python -m mypy src/elspeth/plugins/llm/transform.py src/elspeth/plugins/llm/langfuse.py`
Run: `.venv/bin/python -m ruff check src/elspeth/plugins/llm/transform.py src/elspeth/plugins/llm/langfuse.py`
...
```
filigree close elspeth-rapid-cf10a5 --reason="Azure AI tracing wired into LLMTransform.on_start(), provider validation added, langfuse warning refined"
```
