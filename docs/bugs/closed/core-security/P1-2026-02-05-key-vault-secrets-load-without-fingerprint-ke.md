...
**Date:** 2026-02-06
**Commit/branch:** RC2.3-pipeline-row

### Changes Made

1. **`src/elspeth/core/security/config_secrets.py`** (lines 78-98):
   - Added preflight check at the start of the Key Vault loading path
   - Verifies `ELSPETH_FINGERPRINT_KEY` is available either in environment OR in the secrets mapping
   - Raises `SecretLoadError` with clear remediation message if missing
   - Check happens BEFORE any Key Vault API calls are made

2. **`tests/core/security/test_config_secrets.py`**:
   - Added `test_missing_fingerprint_key_fails_before_keyvault_call`: Verifies the preflight check fails fast before any Key Vault calls
   - Added `test_fingerprint_key_in_mapping_allows_keyvault_load`: Verifies fingerprint key in mapping is accepted
   - Added `test_fingerprint_key_in_env_allows_keyvault_load`: Verifies fingerprint key in environment is accepted
   - Updated all existing Key Vault tests to set `ELSPETH_FINGERPRINT_KEY` in environment via `monkeypatch.setenv()`

### Verification

All 23 tests in `tests/core/security/test_config_secrets.py` pass:
```
.venv/bin/python -m pytest tests/core/security/test_config_secrets.py -v
# Result: 23 passed
```

### Acceptance Criteria Met

- [x] When Key Vault secrets are configured and `ELSPETH_FINGERPRINT_KEY` is missing, `load_secrets_from_config` raises `SecretLoadError` before any secrets are fetched.
- [x] When the fingerprint key is available (env or mapping), secrets load normally and audit recording proceeds without error.
