...
- OS: Linux
- Python version: 3.12+
- Config profile / env vars: Pipeline with fork/expand/coalesce
- Data set or fixture: Any data that results in multiple tokens per row
...
- Notable tool calls or steps: Read lineage.py, recorder.py, docs/design/architecture.md
...
- If multiple tokens exist for a `row_id`, `explain()` should either require a sink for disambiguation or select the unique terminal token
- It should not arbitrarily pick the first token

## Actual Behavior

- `explain()` always selects `tokens[0]` when `row_id` is provided
- Returns lineage for an arbitrary/non-terminal token

## Evidence

- Logs or stack traces: Unknown
- Artifacts (paths, IDs, screenshots):
  - `src/elspeth/core/landscape/lineage.py:75` documents that `row_id` uses the "first token"
  - `src/elspeth/core/landscape/lineage.py:86` selects `tokens[0]`
  ...
  - `docs/design/architecture.md:346` specifies `explain(run_id, row_id, sink, field)` for disambiguation
  - `docs/design/architecture.md:347` says row-only is valid only when the row has a single terminal path
- Minimal repro input (attach or link): Pipeline with fork gate creating multiple tokens for a single row
