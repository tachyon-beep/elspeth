...
fn test_canonical_cbor_format() {
    let frame_id: [u8; 16] = [
        0x62, 0xab, 0x9b, 0x90, 0x25, 0xc1, 0x48, 0xd4, 0x9b, 0x4e, 0x9d, 0x36, 0x95, 0xd8, 0x6a,
        0x72,
    ];
    let level: u32 = 2;
    ...
    let hex_string: String = canonical_bytes
        .iter()
        .map(|b| format!("{:02x}", b))
        .collect();
    ...
    assert_eq!(
        canonical_bytes.len(),
        53,
        "Canonical CBOR should be 53 bytes to match Python"
    );
    ...
    assert_eq!(
        ...
        "Rust canonical CBOR must exactly match Python format"
    );
}
