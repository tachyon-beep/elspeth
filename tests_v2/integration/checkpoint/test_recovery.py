# tests_v2/integration/checkpoint/test_recovery.py
...
Migrated from tests/integration/test_checkpoint_recovery.py.

NOTE: Manual ExecutionGraph construction is intentional here. Checkpoint tests
need specific node IDs matching stored checkpoint records. These IDs must be
deterministic and match what's in the database, which makes
from_plugin_instances() inappropriate (it generates hash-based IDs). See
tests_v2/integration/checkpoint/conftest.py for the same rationale.
"""
