...
class TestPartialFailure:
    ...
    def test_partial_failures_produce_correct_audit_trail(
        self, tmp_path: Path
    ) -> None:
        ...
        assert len(errors) == 3
        error_reasons = {
            json.loads(e.error_details_json)["row_id"] for e in errors
        }
        ...
        with db.engine.connect() as conn:
            token_rows = conn.execute(
                select(tokens_table.c.token_id).join(
                    token_outcomes_table,
    ...
    def test_first_row_failure_doesnt_prevent_rest(
        self, tmp_path: Path
    ) -> None:
    ...
    def test_last_row_failure_doesnt_corrupt_prior_output(
        self, tmp_path: Path
    ) -> None:
