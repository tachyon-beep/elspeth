...
from tests_v2.fixtures.base_classes import (
    _TestSinkBase,
    _TestSourceBase,
    as_sink,
    as_source,
...
class _ResumeSink(_TestSinkBase):
    ...
    def write(self, rows: Any, ctx: Any) -> ArtifactDescriptor:
        ...
        return ArtifactDescriptor.for_file(
            path="memory", size_bytes=0, content_hash="abc"
        )
...
class TestResumeIdempotence:
    ...
    def test_resume_produces_same_result_as_uninterrupted(
        self, tmp_path: Path
    ) -> None:
        ...
        for i, row_data in enumerate(source_data):
            payload_ref = payload_store_b.store(
        ...
        graph_b.add_edge(
            "source", "transform_0", label="continue", mode=RoutingMode.MOVE
        )
        graph_b.add_edge(
            "transform_0", "sink_default", label="continue", mode=RoutingMode.MOVE
        )
        ...
        pre_crash_output = [
            {"id": i, "value": (i + 1) * 10 * 2} for i in range(3)
        ]
...
class TestRetryBehavior:
    ...
    def test_pipeline_with_failed_transform_records_failure(
        self, tmp_path: Path
    ) -> None:
        ...
        with db.engine.connect() as conn:
            errors = conn.execute(
                select(transform_errors_table).where(
                    transform_errors_table.c.run_id == result.run_id
                )
            ).fetchall()
...
class TestCheckpointRecovery:
    ...
    def test_checkpoint_preserves_partial_progress(
        self, test_env: dict[str, Any], mock_graph: ExecutionGraph
    ) -> None:
    ...
    def test_checkpoint_survives_process_restart(
        ...
        assert (
            restored_checkpoint.upstream_topology_hash
            == original_checkpoint.upstream_topology_hash
        )
        assert (
            restored_checkpoint.checkpoint_node_config_hash
            == original_checkpoint.checkpoint_node_config_hash
        )
...
class TestAggregationRecovery:
    ...
    def test_aggregation_state_recovers_after_crash(
        self, test_env: dict[str, Any], mock_graph: ExecutionGraph
    ) -> None:
