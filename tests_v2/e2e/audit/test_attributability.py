# tests_v2/e2e/audit/test_attributability.py
...
class TestAttributability:
    ...
    def test_every_output_row_has_complete_lineage(self, tmp_path: Path) -> None:
        ...
        run_id, db, payload_store, sink = _run_pipeline(
            tmp_path, source_data, transforms=[PassTransform()]
        )
        ...
        for row in rows:
            ...
            assert lineage is not None, (
                f"Row {row.row_id} (index={row.row_index}) has no lineage"
            )

            # Attributability Test from CLAUDE.md
            assert lineage.source_row is not None, (
                f"Row {row.row_id}: lineage.source_row is None"
            )
            assert len(lineage.node_states) > 0, (
                f"Row {row.row_id}: no node_states in lineage"
            )
            assert lineage.source_row.source_data is not None, (
                f"Row {row.row_id}: source_data is None (payload unavailable)"
            )
    ...
    def test_lineage_after_multi_transform(self, tmp_path: Path) -> None:
        ...
        run_id, db, payload_store, sink = _run_pipeline(
            tmp_path, source_data, transforms=transforms
        )
        ...
        for row in rows:
            ...
            assert len(lineage.node_states) >= 2, (
                f"Row {row.row_id}: expected >= 2 node_states, "
                f"got {len(lineage.node_states)}"
            )
            ...
            assert step_indices == sorted(step_indices), (
                f"Row {row.row_id}: step_indices not ordered: {step_indices}"
            )
