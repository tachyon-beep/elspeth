# tests_v2/e2e/audit/test_export_reimport.py
...
class TestExportReimport:
    ...
    def test_export_data_matches_direct_queries(self, tmp_path: Path) -> None:
        ...
        assert len(grouped.get("row", [])) == len(direct_rows), (
            f"Export row count ({len(grouped.get('row', []))}) != "
            f"direct query count ({len(direct_rows)})"
        )

        assert len(grouped.get("node", [])) == len(direct_nodes), (
            f"Export node count ({len(grouped.get('node', []))}) != "
            f"direct query count ({len(direct_nodes)})"
        )

        assert len(grouped.get("edge", [])) == len(direct_edges), (
            f"Export edge count ({len(grouped.get('edge', []))}) != "
            f"direct query count ({len(direct_edges)})"
        )

        assert len(grouped.get("token", [])) == len(direct_tokens), (
            f"Export token count ({len(grouped.get('token', []))}) != "
            f"direct query count ({len(direct_tokens)})"
        )
            ...
            for record in records:
                if "run_id" in record:
                    assert record["run_id"] == run_id, (
                        f"Record type '{record_type}' has wrong run_id: "
                        f"{record['run_id']}"
                    )
