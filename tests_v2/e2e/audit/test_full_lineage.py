# tests_v2/e2e/audit/test_full_lineage.py
...
class TestLineageCompleteness:
    ...
    def test_multi_transform_pipeline_has_complete_lineage(
        self, tmp_path: Path
    ) -> None:
        ...
        for row in rows:
            ...
            assert len(lineage.node_states) >= 2, (
                f"Row {row.row_id} expected >= 2 node_states, got {len(lineage.node_states)}"
            )
...
class TestExplainQueryFunctionality:
    ...
    def test_explain_returns_transform_history(self, tmp_path: Path) -> None:
        ...
        assert lineage is not None
        assert len(lineage.node_states) >= 2, (
            f"Expected >= 2 node_states (2 transforms), got {len(lineage.node_states)}"
        )
        ...
        assert step_indices == sorted(step_indices), (
            f"Node states should be ordered by step_index: {step_indices}"
        )

        # Verify all states completed successfully
        for state in lineage.node_states:
            assert state.status == NodeStateStatus.COMPLETED, (
                f"Node state at step {state.step_index} has status {state.status}"
            )
