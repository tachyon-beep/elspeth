# tests_v2/e2e/pipelines/test_large_pipeline.py
        ...
        with db.engine.connect() as conn:
            ...
            row_count = conn.execute(
                select(func.count()).select_from(rows_table).where(
                    rows_table.c.run_id == result.run_id
                )
            ).scalar()
            ...
            terminal_count = conn.execute(
                select(func.count()).select_from(token_outcomes_table).where(
            ...
            min_idx = conn.execute(
                select(func.min(rows_table.c.row_index)).where(
                    rows_table.c.run_id == result.run_id
                )
            ).scalar()
            max_idx = conn.execute(
                select(func.max(rows_table.c.row_index)).where(
                    rows_table.c.run_id == result.run_id
                )
            ).scalar()
            assert min_idx == 0
