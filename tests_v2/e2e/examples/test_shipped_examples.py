# tests_v2/e2e/examples/test_shipped_examples.py
...
class TestShippedExamples:
    ...
    def test_all_examples_have_settings(self, example_pipeline_dir: Path) -> None:
        ...
        example_dirs = [
            d
            for d in sorted(example_pipeline_dir.iterdir())
    ...
    def test_discover_settings_files(self, example_pipeline_dir: Path) -> None:
        ...
        assert len(settings) >= 20, (
            f"Expected at least 20 settings files, found {len(settings)}"
        )
    ...
    def test_all_settings_are_valid_yaml(self, example_pipeline_dir: Path) -> None:
        ...
        for name, path in settings:
            ...
            assert isinstance(data, dict), (
                f"{name}/{path.name}: settings is not a dict, got {type(data).__name__}"
            )
    ...
    def test_all_settings_have_required_keys(self, example_pipeline_dir: Path) -> None:
        ...
        for name, path in settings:
            ...
            assert not missing, (
                f"{name}/{path.name}: missing required keys {missing}"
            )

    def test_all_settings_have_valid_source_structure(
        self, example_pipeline_dir: Path
    ) -> None:
        ...
        for name, path in settings:
            ...
            assert isinstance(source, dict), (
                f"{name}/{path.name}: source must be a dict"
            )
            assert "plugin" in source, (
                f"{name}/{path.name}: source missing 'plugin' key"
            )

    def test_all_settings_have_valid_sinks_structure(
        self, example_pipeline_dir: Path
    ) -> None:
        ...
        for name, path in settings:
            ...
            assert isinstance(sinks, dict), (
                f"{name}/{path.name}: sinks must be a dict"
            )
            assert len(sinks) > 0, (
                f"{name}/{path.name}: sinks must not be empty"
            )

            # Each sink must have a plugin key
            for sink_name, sink_config in sinks.items():
                assert isinstance(sink_config, dict), (
                    f"{name}/{path.name}: sink '{sink_name}' must be a dict"
                )
                assert "plugin" in sink_config, (
                    f"{name}/{path.name}: sink '{sink_name}' missing 'plugin' key"
                )

    def test_default_sink_references_defined_sink(
        self, example_pipeline_dir: Path
    ) -> None:
        ...
        for name, path in settings:
            ...
            sinks = data.get("sinks", {})
            assert default_sink in sinks, (
                f"{name}/{path.name}: default_sink '{default_sink}' "
                f"not found in sinks {list(sinks.keys())}"
            )

    def test_local_examples_load_via_config_system(
        self, example_pipeline_dir: Path
    ) -> None:
        ...
        settings = self._find_example_settings(example_pipeline_dir)
        local_settings = [
            (name, path) for name, path in settings if not self._needs_env_vars(name)
        ]
        ...
        for name, path in local_settings:
            ...
            assert isinstance(loaded, ElspethSettings), (
                f"{name}/{path.name}: load_settings did not return ElspethSettings"
            )
            # Verify the source plugin is set
            assert loaded.source.plugin, (
                f"{name}/{path.name}: source.plugin is empty"
            )
            # Verify at least one sink exists
            assert len(loaded.sinks) > 0, (
                f"{name}/{path.name}: no sinks defined"
            )

    def test_env_var_examples_are_structurally_valid(
        self, example_pipeline_dir: Path
    ) -> None:
        ...
        settings = self._find_example_settings(example_pipeline_dir)
        env_settings = [
            (name, path) for name, path in settings if self._needs_env_vars(name)
        ]
        ...
        for name, path in env_settings:
            ...
            if "transforms" in data:
                assert isinstance(data["transforms"], list), (
                    f"{name}/{path.name}: transforms must be a list"
                )
                for i, t in enumerate(data["transforms"]):
                    assert isinstance(t, dict), (
                        f"{name}/{path.name}: transform[{i}] must be a dict"
                    )
                    assert "plugin" in t, (
                        f"{name}/{path.name}: transform[{i}] missing 'plugin'"
                    )
    ...
    def test_no_duplicate_sink_names(self, example_pipeline_dir: Path) -> None:
        ...
        for name, path in settings:
            ...
            for sink_name in sinks:
                assert sink_name == sink_name.lower(), (
                    f"{name}/{path.name}: sink name '{sink_name}' is not lowercase"
                )
        ...
        for name, path in settings:
            ...
            for i, gate in enumerate(gates):
                assert "condition" in gate, (
                    f"{name}/{path.name}: gate[{i}] missing 'condition'"
                )
                assert isinstance(gate["condition"], str), (
                    f"{name}/{path.name}: gate[{i}] condition must be a string"
                )
                assert "routes" in gate, (
                    f"{name}/{path.name}: gate[{i}] missing 'routes'"
                )
