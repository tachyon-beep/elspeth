...
def _get_protocol_property_names(protocol: type) -> set[str]:
    ...
    return {
        ...
        if not name.startswith("_") and isinstance(getattr(protocol, name, None), property)
    }
...
class TestRuntimeCheckable:
    ...
    def test_protocol_is_runtime_checkable(self, protocol: type) -> None:
        assert getattr(protocol, "__protocol_attrs__", None) is not None or hasattr(
...
class TestRealImplementations:
    ...
    def test_default_instance_satisfies_protocol(
        self, protocol: type, config_cls: type
    ) -> None:
    ...
    @pytest.mark.parametrize("protocol,config_cls", PROTOCOL_CONFIG_PAIRS)
    def test_real_instance_does_not_satisfy_wrong_protocol(
        self, protocol: type, config_cls: type
    ) -> None:
        instance = config_cls.default()
...
class TestStructuralTyping:
    ...
    def test_missing_property_fails_checkpoint_protocol(self) -> None:
        assert not isinstance(
            _FakeCheckpointMissingFrequency(), RuntimeCheckpointProtocol
        )
    ...
    def test_missing_property_fails_telemetry_protocol(self) -> None:
        assert not isinstance(
            _FakeTelemetryMissingGranularity(), RuntimeTelemetryProtocol
        )
...
class TestProtocolCompleteness:
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(p, PROTOCOL_EXPECTED_PROPERTIES[p], id=p.__name__)
            for p in ALL_PROTOCOLS
        ],
    )
    def test_protocol_has_expected_properties(
        self, protocol: type, expected_props: set[str]
    ) -> None:
        actual = _get_protocol_property_names(protocol)
        assert actual == expected_props, (
            f"{protocol.__name__} properties mismatch.\n"
            f"Missing: {expected_props - actual}\n"
            f"Extra: {actual - expected_props}"
        )
    ...
    def test_no_non_property_public_methods(self, protocol: type) -> None:
        ...
        assert not non_property, (
            f"{protocol.__name__} has non-property public attributes: {non_property}"
        )
    ...
    def test_protocol_properties_have_type_annotations(self, protocol: type) -> None:
        ...
        for prop_name in expected_props:
            ...
            assert ret_type is not inspect.Parameter.empty, (
                f"{protocol.__name__}.{prop_name} is missing a return type annotation"
            )


class TestCrossValidation:
    @pytest.mark.parametrize("protocol,config_cls", PROTOCOL_CONFIG_PAIRS)
    def test_runtime_config_has_all_protocol_properties(
        self, protocol: type, config_cls: type
    ) -> None:
        ...
        assert not missing, (
            f"{config_cls.__name__} is missing protocol properties: {missing}"
        )

    @pytest.mark.parametrize("protocol,config_cls", PROTOCOL_CONFIG_PAIRS)
    def test_protocol_property_types_match_config_annotations(
        self, protocol: type, config_cls: type
    ) -> None:
        protocol_props = _get_protocol_property_names(protocol)
        ...
        for prop_name in protocol_props:
            assert prop_name in config_fields, (
                f"{config_cls.__name__} missing field for {prop_name}"
            )
            ...
            assert proto_name == config_name, (
                f"{config_cls.__name__}.{prop_name} type mismatch: "
                f"protocol expects {proto_name}, config has {config_name}"
            )

    @pytest.mark.parametrize("protocol,config_cls", PROTOCOL_CONFIG_PAIRS)
    def test_default_instance_exposes_all_protocol_properties(
        self, protocol: type, config_cls: type
    ) -> None:
        instance = config_cls.default()
        protocol_props = _get_protocol_property_names(protocol)
