...
def _setup_full(*, run_id: str = "run-1"):
    ...
    recorder.register_edge(
        run_id, "source-0", "transform-1", "continue", RoutingMode.MOVE, edge_id="edge-1"
    )
    ...
    recorder.begin_node_state(
        "tok-1", "transform-1", run_id, 0, {"name": "test"}, state_id="state-1"
    )
    return db, recorder
...
class TestGetRows:
    ...
    def test_rows_scoped_to_run(self):
        ...
        recorder.register_node(
            run_id="run-a", plugin_name="csv", node_type=NodeType.SOURCE,
            plugin_version="1.0", config={}, node_id="src-a", schema_config=_DYNAMIC_SCHEMA,
        )
        recorder.begin_run(config={}, canonical_version="v1", run_id="run-b")
        recorder.register_node(
            run_id="run-b", plugin_name="csv", node_type=NodeType.SOURCE,
            plugin_version="1.0", config={}, node_id="src-b", schema_config=_DYNAMIC_SCHEMA,
        )
...
class TestGetNodeStatesForToken:
    ...
    def test_returns_states_ordered_by_step_index(self):
        ...
        recorder.register_node(
            run_id="run-1", plugin_name="transform2", node_type=NodeType.TRANSFORM,
            plugin_version="1.0", config={}, node_id="transform-2", schema_config=_DYNAMIC_SCHEMA,
        )
        recorder.begin_node_state(
            "tok-1", "transform-2", "run-1", 1, {"name": "test"}, state_id="state-2"
        )
    ...
    def test_orders_by_attempt_within_step(self):
        ...
        recorder.begin_node_state(
            "tok-1", "transform-1", "run-1", 0, {"name": "test"},
            state_id="state-retry", attempt=1,
        )
...
class TestGetRoutingEvents:
    ...
    def test_events_ordered_by_ordinal(self):
        ...
        recorder.register_node(
            run_id="run-1", plugin_name="sink", node_type=NodeType.SINK,
            plugin_version="1.0", config={}, node_id="sink-0", schema_config=_DYNAMIC_SCHEMA,
        )
        recorder.register_edge(
            "run-1", "transform-1", "sink-0", "route_to_sink", RoutingMode.MOVE, edge_id="edge-2"
        )
        recorder.record_routing_event(
            state_id="state-1", edge_id="edge-1", mode=RoutingMode.MOVE, ordinal=1,
        )
        recorder.record_routing_event(
            state_id="state-1", edge_id="edge-2", mode=RoutingMode.MOVE, ordinal=0,
        )
...
class TestGetCalls:
    ...
    def test_calls_ordered_by_call_index(self):
        _, recorder = _setup_full()
        recorder.record_call(
            state_id="state-1", call_index=1, call_type=CallType.LLM,
            status=CallStatus.SUCCESS, request_data={"prompt": "second"},
            response_data={"out": "b"}, latency_ms=50.0,
        )
        recorder.record_call(
            state_id="state-1", call_index=0, call_type=CallType.HTTP,
            status=CallStatus.SUCCESS, request_data={"url": "https://example.com"},
            response_data={"body": "ok"}, latency_ms=75.0,
        )
...
class TestGetRoutingEventsForStates:
    ...
    def test_batch_query_returns_events(self):
        ...
        recorder.begin_node_state(
            "tok-2", "transform-1", "run-1", 0, {"name": "test2"}, state_id="state-2"
        )
        recorder.record_routing_event(
            state_id="state-1", edge_id="edge-1", mode=RoutingMode.MOVE,
        )
        recorder.record_routing_event(
            state_id="state-2", edge_id="edge-1", mode=RoutingMode.MOVE,
        )
...
class TestGetCallsForStates:
    ...
    def test_batch_query_returns_calls(self):
        ...
        recorder.record_call(
            state_id="state-1", call_index=0, call_type=CallType.LLM,
            status=CallStatus.SUCCESS, request_data={"prompt": "a"},
            response_data={"out": "x"}, latency_ms=50.0,
        )
        recorder.record_call(
            state_id="state-2", call_index=0, call_type=CallType.HTTP,
            status=CallStatus.SUCCESS, request_data={"url": "https://example.com"},
            response_data={"body": "ok"}, latency_ms=75.0,
        )
    ...
    def test_single_state_id(self):
        ...
        recorder.record_call(
            state_id="state-1", call_index=0, call_type=CallType.LLM,
            status=CallStatus.SUCCESS, request_data={"prompt": "test"},
            response_data={"out": "ok"}, latency_ms=100.0,
        )
...
class TestGetAllNodeStatesForRun:
    ...
    def test_returns_all_states(self):
        ...
        recorder.create_token("row-2", token_id="tok-2")
        recorder.begin_node_state(
            "tok-2", "transform-1", "run-1", 0, {"b": 2}, state_id="state-2"
        )
    ...
    def test_scoped_to_run(self):
        ...
        recorder.register_node(
            run_id="run-a", plugin_name="tx", node_type=NodeType.TRANSFORM,
            plugin_version="1.0", config={}, node_id="tx-a", schema_config=_DYNAMIC_SCHEMA,
        )
        recorder.create_row("run-a", "src-a", 0, {"v": 1}, row_id="row-a1")
        recorder.create_token("row-a1", token_id="tok-a1")
        recorder.begin_node_state(
            "tok-a1", "tx-a", "run-a", 0, {"v": 1}, state_id="state-a1"
        )
        ...
        recorder.register_node(
            run_id="run-b", plugin_name="tx", node_type=NodeType.TRANSFORM,
            plugin_version="1.0", config={}, node_id="tx-b", schema_config=_DYNAMIC_SCHEMA,
        )
        recorder.create_row("run-b", "src-b", 0, {"v": 2}, row_id="row-b1")
        recorder.create_token("row-b1", token_id="tok-b1")
        recorder.begin_node_state(
            "tok-b1", "tx-b", "run-b", 0, {"v": 2}, state_id="state-b1"
        )
