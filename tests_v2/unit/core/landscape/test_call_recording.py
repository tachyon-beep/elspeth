...
class TestAllocateCallIndex:
    ...
    def test_independent_per_state_id(self):
        ...
        state_a = recorder.begin_node_state(
            "tok-a", "transform-1", "run-1", 0, {"x": 1}, state_id="state-a"
        )
        state_b = recorder.begin_node_state(
            "tok-b", "transform-2", "run-1", 0, {"x": 1}, state_id="state-b"
        )
...
class TestBeginOperation:
    ...
    def test_operation_with_input_data(self):
        ...
        op = recorder.begin_operation(
            "run-1", "source-0", "source_load", input_data={"path": "/data/input.csv"}
        )
...
class TestCompleteOperation:
    ...
    def test_completes_with_failure(self):
        ...
        recorder.complete_operation(
            op_id, "failed", error="File not found", duration_ms=5
        )
    ...
    def test_completes_with_output_data(self):
        ...
        recorder.complete_operation(
            op_id, "completed", output_data={"rows_loaded": 100}, duration_ms=500
        )
...
class TestFindCallByRequestHash:
    ...
    def test_finds_call_by_hash(self):
        ...
        found = recorder.find_call_by_request_hash(
            "run-1", CallType.LLM, original.request_hash
        )
    ...
    def test_returns_none_for_unknown_hash(self):
        ...
        found = recorder.find_call_by_request_hash(
            "run-1", CallType.LLM, "nonexistent-hash"
        )
    ...
    def test_returns_none_for_wrong_call_type(self):
        ...
        found = recorder.find_call_by_request_hash(
            "run-1", CallType.HTTP, original.request_hash
        )
    ...
    def test_sequence_index_for_duplicate_hashes(self):
        ...
        found_0 = recorder.find_call_by_request_hash(
            "run-1", CallType.LLM, calls[0].request_hash, sequence_index=0
        )
        found_1 = recorder.find_call_by_request_hash(
            "run-1", CallType.LLM, calls[0].request_hash, sequence_index=1
        )
