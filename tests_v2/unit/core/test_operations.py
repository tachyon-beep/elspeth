...
from tests_v2.fixtures.factories import make_context
...
def test_track_operation_marks_pending_for_batch_pending_error() -> None:
    ...
    with pytest.raises(BatchPendingError), track_operation(
        recorder=recorder,
        run_id="run-001",
        node_id="node-001",
...
def test_track_operation_marks_failed_for_exception() -> None:
    ...
    with pytest.raises(ValueError, match="boom"), track_operation(
        recorder=recorder,
        run_id="run-001",
        node_id="node-001",
        operation_type="source_load",
...
def test_track_operation_marks_failed_for_base_exception() -> None:
    ...
    with pytest.raises(_Fatal, match="stop-now"), track_operation(
        recorder=recorder,
        run_id="run-001",
        node_id="node-001",
        operation_type="source_load",
...
def test_track_operation_raises_db_error_if_completion_fails_after_success() -> None:
    ...
    with pytest.raises(RuntimeError, match="db write failed"), track_operation(
        recorder=recorder,
        run_id="run-001",
        node_id="node-001",
        operation_type="sink_write",
...
def test_track_operation_does_not_mask_original_exception_when_completion_fails() -> None:
    ...
    with pytest.raises(ValueError, match="original failure"), track_operation(
        recorder=recorder,
        run_id="run-001",
        node_id="node-001",
        operation_type="source_load",
