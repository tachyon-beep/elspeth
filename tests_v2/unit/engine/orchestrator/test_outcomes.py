# tests_v2/unit/engine/orchestrator/test_outcomes.py
...
class TestAccumulateRowOutcomesCompleted:
    ...
    def test_completed_routes_to_branch_sink_when_exists(self) -> None:
        ...
        token = TokenInfo(
            row_id="row-1", token_id="tok-1", row_data=make_row({}), branch_name="branch_a"
        )
        results = [_make_result(RowOutcome.COMPLETED, token=token)]

        accumulate_row_outcomes(
            results, counters, {"output": Mock(), "branch_a": Mock()}, "output", pending
        )
