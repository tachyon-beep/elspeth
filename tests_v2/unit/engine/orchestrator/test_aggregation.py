# tests_v2/unit/engine/orchestrator/test_aggregation.py
...
from tests_v2.fixtures.factories import make_row, make_token_info
...
class TestCheckAggregationTimeouts:
    ...
    def test_downstream_completed_branch_fallback_in_timeout(self) -> None:
        ...
        token = TokenInfo(
            row_id="row-1", token_id="tok-1", row_data=make_row({}), branch_name="unknown"
        )
    ...
    def test_completed_result_branch_fallback_in_timeout(self) -> None:
        ...
        token = TokenInfo(
            row_id="row-1", token_id="tok-1", row_data=make_row({}), branch_name="missing_sink"
        )
        completed = Mock(outcome=RowOutcome.COMPLETED, token=token)
...
class TestFlushRemainingAggregationBuffers:
    ...
    def test_branch_routing_for_completed_tokens(self) -> None:
        ...
        token = TokenInfo(
            row_id="row-1", token_id="tok-1", row_data=make_row({}), branch_name="path_a"
        )
        completed = Mock(outcome=RowOutcome.COMPLETED, token=token)
    ...
    def test_completed_result_branch_fallback_to_default(self) -> None:
        """Completed result with branch not in pending falls back to default."""
        token = TokenInfo(
            row_id="row-1", token_id="tok-1", row_data=make_row({}), branch_name="missing"
        )
        completed = Mock(outcome=RowOutcome.COMPLETED, token=token)
    ...
    def test_branch_routing_falls_back_to_default(self) -> None:
        ...
        token = TokenInfo(
            row_id="row-1", token_id="tok-1", row_data=make_row({}), branch_name="nonexistent"
        )
        completed = Mock(outcome=RowOutcome.COMPLETED, token=token)
