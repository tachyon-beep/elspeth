# tests_v2/performance/scalability/test_many_sinks.py
...
from tests_v2.fixtures.base_classes import as_sink, as_source
    ...
    def test_10_named_sinks(self) -> None:
        ...
        for i in range(10):
            ...
            assert len(sink.results) == 5, (
                f"sink_{i} expected 5 rows, got {len(sink.results)}"
            )
    ...
    def test_50_named_sinks(self) -> None:
        ...
        for i in range(50):
            ...
            assert len(sink.results) == 2, (
                f"sink_{i} expected 2 rows, got {len(sink.results)}"
            )
    ...
    def test_100_named_sinks(self) -> None:
        ...
        for i in range(100):
            ...
            assert len(sink.results) == 1, (
                f"sink_{i} expected 1 row, got {len(sink.results)}"
            )
