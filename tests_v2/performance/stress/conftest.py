# tests_v2/performance/stress/conftest.py
...
class CollectingOutputPort:
    ...
    def emit(
        ...
        if hasattr(result, "exception"):
            with self._lock:
                self.errors.append(
                    ({"reason": "test_error", "error": f"exception: {result.exception}"}, token)
                )
            return
        ...
        if result.status == "success":
            ...
            error_data: TransformErrorReason = (
                result.reason if result.reason is not None else {"reason": "test_error"}
            )
            with self._lock:
...
def verify_audit_integrity(
    ...
    with landscape._db.connection() as conn:
        ...
        result = conn.execute(
            select(token_outcomes_table).where(token_outcomes_table.c.run_id == run_id)
        )
