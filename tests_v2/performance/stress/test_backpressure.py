# tests_v2/performance/stress/test_backpressure.py
...
class TestTelemetryBackpressure:
    ...
    def test_telemetry_emission_under_load(self) -> None:
        ...
        try:
            ...
            assert exporter.count == num_events, (
                f"Expected {num_events} events, got {exporter.count}"
            )

            # Verify no drops
            metrics = manager.health_metrics
            assert metrics["events_dropped"] == 0, (
                f"Events were dropped: {metrics['events_dropped']}"
            )
    ...
    def test_backpressure_graceful_degradation(self) -> None:
        ...
        try:
            ...
            assert metrics["events_dropped"] > 0, (
                "Expected some drops with slow exporter; "
                f"emitted={metrics['events_emitted']}, dropped={metrics['events_dropped']}"
            )
