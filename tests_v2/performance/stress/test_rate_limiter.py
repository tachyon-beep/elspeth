# tests_v2/performance/stress/test_rate_limiter.py
...
class TestRateLimiterStress:
    ...
    def test_rate_limiter_burst_recovery(self) -> None:
        ...
        with RateLimiter(
            "burst_test", requests_per_minute=rpm, window_ms=window_ms
        ) as limiter:
            ...
            assert acquired_count == rpm, (
                f"Expected {rpm} successful acquires, got {acquired_count}"
            )
    ...
    def test_rate_limiter_fairness_under_load(self) -> None:
        ...
        with RateLimiter(
            "fairness_test", requests_per_minute=rpm, window_ms=window_ms
        ) as limiter:
            ...
            threads = [
                threading.Thread(target=worker, args=(i,), name=f"fair-worker-{i}")
            ...
            for tid in range(num_threads):
                assert thread_counts[tid] == acquires_per_thread, (
                    f"Thread {tid} got {thread_counts[tid]} acquires, "
                    f"expected {acquires_per_thread} "
                    f"(counts: {dict(thread_counts)})"
                )
