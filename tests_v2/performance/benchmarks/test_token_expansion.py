...
class TestExpandTokenDeepCopyPerformance:
    ...
    def _create_medium_row() -> dict[str, Any]:
        ...
        return {
            ...
            "payload": {
                "items": [
                    {"sku": f"SKU-{i}", "quantity": i * 10, "price": i * 1.5}
                    for i in range(100)
                ],
            },
            "nested": {
    ...
    def test_deepcopy_small_rows_baseline(self) -> None:
        ...
        assert us_per_copy < 10, (
            f"Small row deepcopy: {us_per_copy:.2f}us (expected < 10us)"
        )
    ...
    def test_deepcopy_medium_rows_baseline(self) -> None:
        ...
        assert us_per_copy < 200, (
            f"Medium row deepcopy: {us_per_copy:.2f}us (expected < 200us)"
        )
    ...
    def test_deepcopy_large_rows_baseline(self) -> None:
        ...
        assert ms_per_copy < 5, (
            f"Large row deepcopy: {ms_per_copy:.2f}ms (expected < 5ms)"
        )
    ...
    def test_expand_token_simulation_small(self) -> None:
        ...
        assert ms_per_expansion < 1, (
            f"10x small expansion: {ms_per_expansion:.2f}ms (expected < 1ms)"
        )
    ...
    def test_expand_token_simulation_high_fanout(self) -> None:
        ...
        assert ms_per_expansion < 10, (
            f"100x small expansion: {ms_per_expansion:.2f}ms (expected < 10ms)"
        )
    ...
    def test_expand_token_simulation_large_high_fanout(self) -> None:
        ...
        assert ms_per_expansion < 500, (
            f"50x large expansion: {ms_per_expansion:.2f}ms (expected < 500ms)"
        )
    ...
    def test_shallow_vs_deep_copy_comparison(self) -> None:
        ...
        assert deep_us < 500, (
            f"Deep copy too slow: {deep_us:.2f}us (expected < 500us)"
        )
    ...
    def test_memory_amplification_factor(self) -> None:
        ...
        def get_deep_size(obj: Any, seen: set[int] | None = None) -> int:
            ...
            if isinstance(obj, dict):
                size += sum(
                    get_deep_size(k, seen) + get_deep_size(v, seen)
        ...
        assert expected_min <= amplification <= expected_max, (
            f"Memory amplification {amplification:.2f}x outside expected range "
