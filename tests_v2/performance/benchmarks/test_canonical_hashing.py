...
def test_hash_throughput_small_rows() -> None:
    ...
    assert rows_per_sec > 5000, (
        f"Small row hashing: {rows_per_sec:.0f} rows/sec (expected > 5000)"
    )
...
def test_hash_throughput_medium_rows() -> None:
    ...
    assert rows_per_sec > 500, (
        f"Medium row hashing: {rows_per_sec:.0f} rows/sec (expected > 500)"
    )
...
def test_hash_throughput_large_rows() -> None:
    ...
    assert rows_per_sec > 50, (
        f"Large row hashing: {rows_per_sec:.0f} rows/sec (expected > 50)"
    )
...
def test_hash_consistency_across_iterations() -> None:
    ...
    assert rows_per_sec > 200, (
        f"Hash consistency check: {rows_per_sec:.0f} rows/sec (expected > 200)"
    )
