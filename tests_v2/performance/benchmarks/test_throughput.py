...
from tests_v2.fixtures.plugins import CollectSink, ListSource, PassTransform
...
def test_throughput_100_rows_passthrough() -> None:
    ...
    assert timing.wall_seconds < 5.0, (
        f"100-row pipeline took {timing.wall_seconds:.2f}s "
        f"({rows_per_sec:.0f} rows/sec, expected > 20 rows/sec)"
    )
...
def test_throughput_1000_rows_passthrough() -> None:
    ...
    assert timing.wall_seconds < 30.0, (
        f"1000-row pipeline took {timing.wall_seconds:.2f}s "
        f"({rows_per_sec:.0f} rows/sec, expected > 33 rows/sec)"
    )
...
def test_throughput_with_multiple_transforms() -> None:
    ...
    assert timing.wall_seconds < 10.0, (
        f"100-row 5-transform pipeline took {timing.wall_seconds:.2f}s "
        f"({rows_per_sec:.0f} rows/sec, expected > 10 rows/sec)"
    )
...
def test_throughput_fork_pipeline() -> None:
    ...
    assert timing.wall_seconds < 10.0, (
        f"100-row fork pipeline took {timing.wall_seconds:.2f}s "
        f"({rows_per_sec:.0f} rows/sec, expected > 10 rows/sec)"
    )
