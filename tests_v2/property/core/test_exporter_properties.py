# tests_v2/property/core/test_exporter_properties.py
...
class TestExportSigningProperties:
    ...
    def test_signing_is_deterministic_for_data_records(self, key: bytes) -> None:
        ...
        """
        records1 = list(
            _make_exporter_with_data(signing_key=key, num_records=2).export_run("run-1", sign=True)
        )
        records2 = list(
            _make_exporter_with_data(signing_key=key, num_records=2).export_run("run-1", sign=True)
        )
...
def _make_exporter_with_data(
    ...
    """
    test_records = [
        {"record_type": f"test_{i}", "data": f"value_{i}", "index": i}
    ...
    original_iter = exporter._iter_records
