# tests_v2/property/core/test_operations_properties.py
...
from elspeth.core.operations import OperationHandle, track_operation
...
class TestContextRestorationProperties:
    ...
    def test_batch_pending_restores_context(self, prev_op_id: str | None) -> None:
        ...
        with pytest.raises(BatchPendingError):
            with track_operation(recorder, "run-1", "node-1", "sink_write", ctx):
...
class TestOutputDataProperties:
    ...
    def test_handle_exposes_operation(self) -> None:
        ...
        with track_operation(
            recorder, "run-1", "node-1", "source_load", ctx,
...
class TestNestingProperties:
    ...
    def test_nested_operations_restore_stack(self, outer_prev: str | None) -> None:
        ...
        with track_operation(recorder, "run-1", "source", "source_load", ctx) as outer:
            outer_op_id = ctx.operation_id
            assert outer_op_id is not None

            with track_operation(recorder, "run-1", "sink", "sink_write", ctx) as inner:
