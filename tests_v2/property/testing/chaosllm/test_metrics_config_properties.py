# tests_v2/property/testing/chaosllm/test_metrics_config_properties.py
...
class TestGetBucketUtc:
    ...
    def test_bucket_is_aligned(
        self, hour: int, minute: int, second: int, bucket_sec: int
    ) -> None:
        ...
        assert total_seconds % bucket_sec == 0, (
            f"Bucket {bucket} not aligned to {bucket_sec}s boundary"
        )
    ...
    @settings(max_examples=200)
    def test_bucket_does_not_exceed_timestamp(
        self, hour: int, minute: int, second: int, bucket_sec: int
    ) -> None:
        ...
        bucket_dt = datetime.fromisoformat(bucket)
        assert bucket_dt <= original_dt, (
            f"Bucket {bucket} exceeds original {ts}"
        )
    ...
    @settings(max_examples=200)
    def test_timestamp_within_one_bucket_of_result(
        self, hour: int, minute: int, second: int, bucket_sec: int
    ) -> None:
        ...
        assert 0 <= diff < bucket_sec, (
            f"Diff {diff}s not in [0, {bucket_sec}): ts={ts}, bucket={bucket}"
        )
...
class TestClassifyOutcome:
    ...
    @given(
        ...
        error_type=st.one_of(
            st.just(None),
            st.sampled_from([
                "timeout", "connection_failed", "connection_stall",
                "connection_reset", "rate_limit", "unknown",
            ]),
        ),
        outcome=st.sampled_from(["success", "error_injected", "error_malformed"]),
    )
    @settings(max_examples=200)
    def test_classification_returns_seven_booleans(
        self, status_code: int | None, error_type: str | None, outcome: str
    ) -> None:
    ...
    @given(
        error_type=st.sampled_from([
            "timeout", "connection_failed", "connection_stall", "connection_reset",
        ])
    )
...
class TestDeepMerge:
    ...
    @settings(max_examples=50)
    def test_nested_merge_preserves_other_keys(
        self, a_val: int, b_val: int, c_val: int
    ) -> None:
    ...
    @settings(max_examples=50)
    def test_nested_override_replaces_value(
        self, a_val: int, a_new: int, b_val: int
    ) -> None:
