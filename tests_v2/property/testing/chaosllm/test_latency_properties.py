# tests_v2/property/testing/chaosllm/test_latency_properties.py
    ...
    def test_simulate_within_bounds(self, base_ms: int, jitter_ms: int, seed: int) -> None:
        ...
        for _ in range(50):
            ...
            assert lower - 1e-9 <= delay <= upper + 1e-9, (
                f"Delay {delay:.6f}s outside bounds [{lower:.6f}, {upper:.6f}]"
            )
    ...
    def test_slow_response_within_range(self, min_sec: int, max_sec: int, seed: int) -> None:
        ...
        for _ in range(50):
            ...
            assert min_sec <= delay <= max_sec, (
                f"Slow response delay {delay:.3f} outside [{min_sec}, {max_sec}]"
            )
...
class TestStatisticalProperties:
    ...
    def test_mean_converges_to_base(self, seed: int) -> None:
        ...
        assert abs(mean - expected) < expected * 0.10, (
            f"Mean {mean:.6f} too far from expected {expected:.6f}"
        )
