# tests_v2/property/plugins/transforms/azure/test_azure_safety_properties.py
...
class TestContentSafetyThresholdsValidation:
    ...
    def test_extra_fields_rejected(self) -> None:
        ...
        with pytest.raises(pydantic.ValidationError, match="extra"):
            ContentSafetyThresholds(
                hate=2, violence=2, sexual=2, self_harm=2,
...
class TestCheckThresholdsProperties:
    ...
    def test_threshold_result_matches_manual_check(
        self, config: dict[str, int], analysis: dict[str, int]
    ) -> None:
    ...
    @settings(max_examples=200)
    def test_single_violation_triggers_full_report(
        self, config: dict[str, int], violating_cat: str
    ) -> None:
    ...
    @given(config=threshold_configs)
    @settings(max_examples=200)
    def test_max_severity_always_triggers_when_threshold_under_max(
...
class TestContentSafetyFailClosed:
    ...
    @given(
        unknown_category=st.text(min_size=1, max_size=20).filter(
            lambda s: s not in _AZURE_CATEGORY_MAP
        ),
    )
    @settings(max_examples=100)
