# tests_v2/property/plugins/web_scrape/test_ssrf_properties.py
...
def blocked_ipv6_from_range(draw: st.DrawFn) -> tuple[str, ipaddress.IPv6Network]:
    ...
    ipv6_ranges = [
        ...
        if r.version == 6 and r != ipaddress.ip_network("::ffff:0:0/96")
    ]
...
class TestBlockedIPv4Ranges:
    ...
    def test_blocked_ipv4_via_full_validation(self, data: tuple[str, ipaddress.IPv4Network]) -> None:
        ...
        ip_str, _ = data
        with patch("socket.getaddrinfo", side_effect=_mock_dns_to([ip_str])):
...
class TestIPv4MappedIPv6Bypass:
    ...
    def test_ipv4_mapped_via_full_validation(self, mapped_ip: str) -> None:
        ...
        with patch("socket.getaddrinfo", side_effect=_mock_dns_to([mapped_ip])):
            with pytest.raises(SSRFBlockedError):
...
class TestMultiHomedHost:
    ...
    @settings(max_examples=200)
    def test_mixed_ips_blocked_if_any_unsafe(
        self, blocked: tuple[str, ipaddress.IPv4Network], safe: str
    ) -> None:
        """Property: Mixed safe+blocked IPs are rejected (fail-closed)."""
        blocked_ip, _ = blocked
        # DNS returns both safe and blocked IPs
        with patch("socket.getaddrinfo", side_effect=_mock_dns_to([safe, blocked_ip])):
            with pytest.raises(SSRFBlockedError):
    ...
    def test_order_doesnt_matter(
        ...
        blocked_ip, _ = blocked
        # Blocked IP first
        with patch("socket.getaddrinfo", side_effect=_mock_dns_to([blocked_ip, safe])):
            with pytest.raises(SSRFBlockedError):
                validate_url_for_ssrf("http://multihomed.example.com/")
