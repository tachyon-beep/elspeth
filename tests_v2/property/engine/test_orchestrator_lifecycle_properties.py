# tests_v2/property/engine/test_orchestrator_lifecycle_properties.py
...
class TestFlushResultMonoidProperties:
    ...
    def test_add_preserves_all_counter_fields(
        self, a: AggregationFlushResult, b: AggregationFlushResult
    ) -> None:
    ...
    @given(a=aggregation_flush_results(), b=aggregation_flush_results())
    @settings(max_examples=100)
    def test_routed_destinations_merge_is_additive(
        self, a: AggregationFlushResult, b: AggregationFlushResult
    ) -> None:
...
class TestAccumulateFlushResultProperties:
    ...
    def test_accumulate_adds_to_existing(
        self, counters: ExecutionCounters, flush: AggregationFlushResult
    ) -> None:
    ...
    @given(counters=execution_counters(), flush=aggregation_flush_results())
    @settings(max_examples=100)
    def test_accumulate_merges_routed_destinations(
        self, counters: ExecutionCounters, flush: AggregationFlushResult
    ) -> None:
    ...
    @given(counters=execution_counters())
    @settings(max_examples=100)
    def test_accumulate_does_not_touch_rows_processed(
    ...
    @given(counters=execution_counters())
    @settings(max_examples=100)
    def test_accumulate_does_not_touch_coalesce_failed(
        self, counters: ExecutionCounters
    ) -> None:
    ...
    def test_sequential_accumulation_equals_summed(
        self, counters: ExecutionCounters, flushes: list[AggregationFlushResult]
    ) -> None:
...
class TestAccumulateRowOutcomesProperties:
    ...
    def test_mixed_outcomes_conservation(
        self, completed: int, failed: int, routed: int, quarantined: int
    ) -> None:
        ...
        results.extend(
            _make_row_result(RowOutcome.ROUTED, sink_name="alerts") for _ in range(routed)
        )
...
class TestRunResultFieldProperties:
    ...
    def test_run_result_field_coverage(self, counters: ExecutionCounters) -> None:
        ...
        for field_name in counter_fields:
            ...
            assert run_value == counter_value, (
                f"Field {field_name} mismatch: RunResult={run_value}, Counters={counter_value}"
            )
