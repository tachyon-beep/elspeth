# tests_v2/property/engine/test_token_properties.py
...
from tests_v2.strategies.ids import multiple_branches
from tests_v2.strategies.json import row_data
from tests_v2.strategies.mutable import deeply_nested_data, mutable_nested_data
...
def _create_mock_recorder_for_expand(count: int) -> MagicMock:
    ...
    mock_recorder = MagicMock()
    children = [
        MagicMock(token_id=f"expanded_{i}", expand_group_id="expand_1")
...
class TestExpandIsolationProperties:
    ...
    def test_expand_creates_isolated_copies(self, row_data: dict[str, Any], count: int) -> None:
        ...
        for i in range(len(children)):
            for j in range(i + 1, len(children)):
                assert children[i].row_data is not children[j].row_data, (
                    f"Expanded children {i} and {j} share PipelineRow instance! "
                    f"expand_token must deepcopy each row independently."
                )
    ...
    def test_expand_isolates_deeply_nested_data(self, row_data: Any, count: int) -> None:
        ...
        for i in range(len(children)):
            for j in range(i + 1, len(children)):
                assert children[i].row_data is not children[j].row_data, (
                    f"Deep nesting: Expanded children {i} and {j} share PipelineRow!"
                )
...
class TestExpandParentPreservationProperties:
    ...
    def test_expand_preserves_parent_data(self, row_data: dict[str, Any], count: int) -> None:
        ...
        for i, child in enumerate(children):
            assert child.row_data is not parent.row_data, (
                f"Expanded child {i} shares PipelineRow with parent!"
            )
    ...
    def test_expand_children_have_correct_metadata(self, row_data: dict[str, Any], count: int) -> None:
        ...
        for i, child in enumerate(children):
            ...
            assert child.row_id == parent.row_id, (
                f"Expanded child {i} has wrong row_id: {child.row_id} != {parent.row_id}"
            )
