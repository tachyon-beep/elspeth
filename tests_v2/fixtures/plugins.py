...
from tests_v2.fixtures.base_classes import _TestSchema, _TestSinkBase, _TestSourceBase
    ...
    def process(self, row: Any, ctx: Any) -> TransformResult:
        ...
        if self._call_count == self._error_on:
            return TransformResult.error(
                {"reason": "nth_error", "n": self._error_on}, retryable=True
            )
...
class RoutingGate:
    ...
    def __init__(
        self, field: str, route_map: dict[str, str], default: str = "continue"
    ) -> None:
        self._field = field
