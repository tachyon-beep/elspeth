    ...
    *,
    ...
    for branch_label, transforms in branches.items():
        ...
        for t_name in transforms:
            graph.add_node(
                t_name, node_type=NodeType.TRANSFORM, plugin_name="test-transform", config={}
            )
    ...
    *,
    ...
    return {
        "coalesce_context": {
            ...
            "branches_lost": {},
            "arrival_order": [
                {"branch": b, "arrival_offset_ms": float(i * 50)} for i, b in enumerate(arrived)
            ],
...
def make_batch_checkpoint(
    *,
    ...
    return {
        ...
        "row_mapping": {
            f"custom_{i}": {"id": i, "text": f"row-{i}"} for i in range(row_count)
        },
        "template_errors": [],
        "requests": {
            f"custom_{i}": {"model": "gpt-4", "messages": []} for i in range(row_count)
        },
    }
