...
def _build_config_from_marker(
    ...
    for key in [
        ...
        "timeout_pct", "connection_reset_pct", "slow_response_pct",
        "invalid_json_pct", "truncated_pct", "empty_body_pct",
        "missing_fields_pct", "wrong_content_type_pct", "selection_mode",
    ]:
...
def chaosllm_server(
...
) -> Generator[ChaosLLMFixture, None, None]:
    ...
    fixture = ChaosLLMFixture(
        client=client, server=server, metrics_db_path=metrics_db_path
    )
    yield fixture
    client.close()
